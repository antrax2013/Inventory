...
var invChannel = new InventoryChannel(new CommandHandler());
await BenchmarkHelper.Run("Mono Channel", Iterations, () => DistributedAddOrRemove(invChannel, 1));

var inv4Channel = new InventoryChannel(new CommandHandler(), 4);
await BenchmarkHelper.Run("4 Channels", Iterations, () => DistributedAddOrRemove(inv4Channel, 1));
